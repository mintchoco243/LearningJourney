/* =============================================================
   Extended Quiz — Steps 4, 5, 6 (Learning Style, Availability, Trainer Preference)
   ============================================================= */
(function () {
  "use strict";
  const { Icon } = window.GLHUI;
  const { useGame, scoreQuiz } = window.GLHEngine;
  const D = window.GLH_DATA;

  // Step 4: Learning Style (multi-select tiles, multi-select)
  function Step4LearningStyle(props) {
  ...
  function Step5Availability(props) {
    const [selected, setSelected] = React.useState(props.value || "");
  ...
  function Step6TrainerPreference(props) {
  ...
  window.GLHScreens = Object.assign(window.GLHScreens || {}, {
    Step4LearningStyle, Step5Availability, Step6TrainerPreference
  });
})();
